...
def main():
    try:
        # Recuperiamo il calendario (User-Agent serve per non essere bloccati)
        response = requests.get(FF_FEED, headers={'User-Agent': 'Mozilla/5.0'})
        root = ET.fromstring(response.content)
        
        # Data di oggi (formato Forex Factory: 02-27-2026)
        ...
        for item in root.findall('event'):
            ...
            # FILTRO: Solo news di OGGI e solo Alto Impatto (Red)
        ...
        if not news_found:
            print("Nessuna news ad alto impatto per oggi.")
...
def send_to_discord(title, country, time):
    payload = {
        "embeds": [{
            ...
            "description": f"**Evento:** {title}\n**Ora:** {time} (EST/New York)",
            "color": 15158332, # Rosso
            "footer": {"text": "Dati da Forex Factory"}
        }]
    }
